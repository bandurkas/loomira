...
function ColorSlider({ onSelect, selected }: { onSelect: (id: string) => void; selected: string }) {
  ...
  const isLight = item.id === 'White' || item.id === 'Grey' || item.id === 'Mix';

  return (
    <div className="relative w-full overflow-hidden border border-ink/15 rounded-3xl shadow-[0_12px_40px_rgba(14,14,14,0.13)]">
      {/* Main slide */}
      <div
        ref={trackRef}
          ...
          >
            ...
            <div className="flex flex-col justify-center p-8 md:p-16 gap-6">
              <div className="space-y-3">
                <div className={`text-[11px] font-black tracking-[0.3em] uppercase ${isLight ? 'text-ink/60' : 'text-white/60'}`}>
                ...
                <p className={`text-[15px] font-bold ${isLight ? 'text-ink/75' : 'text-white/80'}`}>{item.sub}</p>
              </div>
              ...
              <button
                ...
                className={`self-start inline-flex items-center gap-2 px-6 py-3 border-2 font-black text-[13px] uppercase tracking-wider transition-all hover:-translate-y-0.5 ${
                  isLight
                    ? 'bg-ink text-[#D1F242] border-ink shadow-[0_4px_18px_rgba(209,242,66,0.4)]'
              ...
              <div className="flex gap-3 mt-2">
                {SLIDER_ITEMS.map((s, i) => (
                  <button
                    key={s.id}
                    onClick={() => goTo(i)}
                    className={`w-8 h-2 border border-ink/30 rounded-full transition-all ${i === active ? 'bg-[#D1F242] w-12' : (isLight ? 'bg-ink/20' : 'bg-white/30')}`}
                  />
                ))}
              </div>
            </div>
      ...
      <button
        ...
        className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-ink/80 border border-ink/20 flex items-center justify-center rounded-full backdrop-blur-sm shadow-[0_3px_14px_rgba(209,242,66,0.35)] hover:-translate-y-[calc(50%+2px)] transition-transform z-10"
        aria-label="Previous"
      >
      ...
      <button
        onClick={() => goTo(active + 1)}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-ink/80 border border-ink/20 flex items-center justify-center rounded-full backdrop-blur-sm shadow-[0_3px_14px_rgba(209,242,66,0.35)] hover:-translate-y-[calc(50%+2px)] transition-transform z-10"
        aria-label="Next"
      >
      ...
      {/* Thumbnail strip */}
      <div className="grid grid-cols-4 border-t border-ink/20">
        {SLIDER_ITEMS.map((s, i) => (
          <button
            key={s.id}
            onClick={() => goTo(i)}
            className={`relative py-4 px-3 flex flex-col items-center gap-2 border-r last:border-r-0 border-ink/20 transition-all ${
    ...
    <div className="min-h-screen bg-cream text-ink font-sans antialiased selection:bg-ink selection:text-cream">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-cream/92 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 sm:px-10 flex items-center justify-between h-[62px]">
          ...
          <a href="#pricing" className="text-[12px] font-black uppercase bg-[#D1F242] text-ink px-5 py-2.5 border border-ink/30 rounded-full shadow-[0_6px_20px_rgba(14,14,14,0.10)] hover:-translate-y-0.5 transition-all tracking-wider">
            Beli Sekarang
          </a>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 max-w-6xl mx-auto">
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7, ease:EASE }}>
            <a href="#pricing"
              className="group inline-flex items-center gap-3 bg-[#D1F242] text-ink px-8 py-4 border border-ink/30 rounded-full shadow-[0_8px_28px_rgba(14,14,14,0.11)] font-black text-[16px] uppercase tracking-[0.08em] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(14,14,14,0.13)] transition-all">
              AMANKAN PROMONYA
      ...
      <main className="max-w-6xl mx-auto px-6 sm:px-10 pb-28 md:pb-0">
        ...
        <section className="py-16 md:py-24 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <Reveal className="space-y-6">
            <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Masalah Umum</span>
          ...
          <Reveal delay={0.12} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="bg-white border border-ink/20 p-7 space-y-4 rounded-3xl shadow-[0_8px_28px_rgba(14,14,14,0.11)]">
              <div className="w-10 h-10 bg-ink flex items-center justify-center border border-ink/20 rounded-full"><X size={18} className="text-white" /></div>
            ...
            <div className="bg-[#D1F242] border border-ink/20 p-7 space-y-4 rounded-3xl shadow-[0_8px_28px_rgba(14,14,14,0.11)]">
              <div className="w-10 h-10 bg-ink flex items-center justify-center border border-ink/20 rounded-full"><Check size={18} className="text-[#D1F242]" /></div>
              <div className="space-y-2">
        ...
        <section id="features" className="py-16 md:py-24 space-y-12">
          <Reveal className="max-w-2xl space-y-4">
            <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Keunggulan</span>
            <h2 className="font-display font-black text-[clamp(2.5rem,5vw,4rem)] leading-[1] text-ink uppercase tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((f,i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div className="group bg-ink border border-ink/20 p-8 space-y-8 hover:-translate-y-2 transition-all duration-300 h-full rounded-3xl shadow-[0_8px_28px_rgba(209,242,66,0.45)]">
                  <div className="w-16 h-16 bg-[#D1F242] flex items-center justify-center border border-ink/20 rounded-full shadow-[0_6px_20px_rgba(14,14,14,0.10)] group-hover:scale-110 transition-transform duration-300">
        ...
        <section id="gallery" className="py-16 md:py-24">
          <Reveal className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
            <div className="space-y-4">
              <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Koleksi</span>
          ...
          <Reveal>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 lg:h-[540px]">
              ...
              <div className="lg:col-span-5 relative overflow-hidden bg-[#ECEAE7] group cursor-pointer aspect-[4/5] lg:aspect-auto border border-ink/15 rounded-3xl shadow-[0_12px_40px_rgba(14,14,14,0.13)]">
              ...
              <div className="lg:col-span-7 grid grid-rows-[3fr_2fr] gap-3.5 lg:h-[540px]">
                ...
                <div className="relative overflow-hidden bg-[#F0EFEA] group cursor-pointer aspect-[16/7] lg:aspect-auto border border-ink/15 rounded-3xl shadow-[0_8px_28px_rgba(14,14,14,0.11)]">
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="relative overflow-hidden bg-[#ECEAE5] group cursor-pointer aspect-square lg:aspect-auto border border-ink/15 rounded-3xl shadow-[0_6px_20px_rgba(14,14,14,0.10)]">
                  ...
                  <div className="relative overflow-hidden bg-[#EDE8DF] group cursor-pointer aspect-square lg:aspect-auto border border-ink/15 rounded-3xl shadow-[0_6px_20px_rgba(14,14,14,0.10)]">
          ...
          <Reveal delay={0.08} className="mt-3.5">
            <div className="grid grid-cols-2 md:grid-cols-4 overflow-hidden border border-ink/20 bg-[#D1F242] rounded-3xl shadow-[0_6px_20px_rgba(14,14,14,0.10)]">
        ...
        <section id="reviews" className="py-16 md:py-24 space-y-12">
          <Reveal className="space-y-4">
            <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Ulasan</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {REVIEWS.map((r,i) => (
              <Reveal key={i} delay={i * 0.09}>
                <div className="bg-[#F4F4F4] border border-ink/15 p-7 space-y-5 h-full flex flex-col rounded-3xl shadow-[0_6px_20px_rgba(14,14,14,0.10)] hover:-translate-y-1 transition-all duration-300">
        ...
        <section className="py-16 md:py-24">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            ...
            <Reveal className="space-y-6 lg:sticky lg:top-24">
              <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Perbandingan</span>
              <h2 className="font-display font-black text-[clamp(2.5rem,5vw,4rem)] leading-[1] text-ink uppercase tracking-tight">
              ...
              <a href="#pricing"
                className="inline-flex items-center gap-2 bg-[#D1F242] text-ink px-6 py-3 border border-ink/30 rounded-full shadow-[0_6px_20px_rgba(14,14,14,0.10)] font-black text-[13px] uppercase tracking-wider hover:-translate-y-0.5 transition-all group">
                Lihat Penawaran
            ...
            <Reveal delay={0.1}>
              <div className="bg-white border border-ink/15 overflow-hidden rounded-3xl shadow-[0_12px_40px_rgba(14,14,14,0.13)]">
                ...
                {COMPARISON.map((feat,i) => (
                  <div key={i} className="grid grid-cols-12 px-6 py-4 border-b-2 last:border-b-0 border-ink/10 items-center hover:bg-[#D1F242]/10 transition-colors">
                    ...
                    <div className="col-span-3 flex justify-center">
                      <div className="w-7 h-7 bg-[#D1F242] flex items-center justify-center border border-ink/20 rounded-full">
                        <Check size={13} strokeWidth={3} className="text-ink" />
                      </div>
                    </div>
                    <div className="col-span-3 flex justify-center">
                      <div className="w-7 h-7 bg-ink/5 flex items-center justify-center border border-ink/10 rounded-full">
        ...
        <section className="py-16 md:py-24">
          <Reveal className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div className="space-y-3">
                <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Pilih Warna</span>
        ...
        <section id="pricing" className="py-16 md:py-24 space-y-12">
          <Reveal className="space-y-4">
            <span className="inline-block bg-ink text-[#D1F242] font-black px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-full shadow-[0_4px_18px_rgba(209,242,66,0.4)]">Penawaran</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {BUNDLES.map(b => (
                <button key={b.id} onClick={() => setBundle(b.id)}
                  className={`relative p-7 border border-ink/20 text-left transition-all duration-300 cursor-pointer hover:-translate-y-1 rounded-3xl ${
                    bundle === b.id
                      ? 'bg-ink shadow-[0_8px_28px_rgba(209,242,66,0.45)]'
                      : 'bg-white shadow-[0_8px_28px_rgba(14,14,14,0.11)] hover:shadow-[0_8px_28px_rgba(209,242,66,0.45)]'
                  }`}
                >
                  {b.featured && (
                    <span className={`absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-black tracking-[0.14em] uppercase px-3.5 py-1 whitespace-nowrap border border-ink/30 rounded-full ${
          ...
          <Reveal delay={0.07}>
            <div className="bg-white border border-ink/15 p-7 space-y-7 rounded-3xl shadow-[0_8px_28px_rgba(14,14,14,0.11)]">
                ...
                <div className="flex gap-3 flex-wrap">
                  {['S','M','L','XL'].map(s => (
                    <button key={s} onClick={() => setSize(s)}
                      className={`relative h-11 min-w-[52px] px-4 text-[13px] font-black border border-ink/25 transition-all cursor-pointer uppercase rounded-full ${
                        size === s
                          ? 'bg-ink text-cream shadow-[0_3px_14px_rgba(209,242,66,0.35)]'
          ...
          <Reveal delay={0.1}>
            <div className="space-y-4">
              <motion.a href={waUrl} target="_blank"
                ...
                className="group w-full bg-[#D1F242] text-ink py-[18px] border border-ink/30 rounded-full shadow-[0_8px_28px_rgba(14,14,14,0.11)] font-black text-[18px] uppercase tracking-[0.05em] flex items-center justify-center gap-3 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_rgba(14,14,14,0.13)] transition-all"
              >
        ...
        <Reveal>
          <section className="pt-6 pb-16 md:pt-8 md:pb-24">
            <div className="bg-white border border-ink/15 px-8 md:px-12 py-10 flex flex-col md:flex-row rounded-3xl items-center gap-7 text-center md:text-left shadow-[0_8px_28px_rgba(209,242,66,0.45)]">
              <div className="w-16 h-16 min-w-[64px] min-h-[64px] bg-[#D1F242] flex items-center justify-center border border-ink/20 rounded-3xl shadow-[0_6px_20px_rgba(14,14,14,0.10)]">
              ...
              <button onClick={() => setModal(true)}
                className="shrink-0 text-[12px] font-black text-ink border border-ink/25 px-5 py-2.5 uppercase tracking-wider rounded-full hover:bg-[#D1F242] hover:shadow-[0_6px_20px_rgba(14,14,14,0.10)] transition-all duration-300 cursor-pointer">
                Baca S&K
              </button>
            </div>
          </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden px-4 py-3.5 bg-ink border-t-2 border-[#D1F242]">
        <a href={waUrl} target="_blank"
          className="w-full bg-[#D1F242] text-ink py-4 flex items-center justify-center font-black text-[15px] uppercase tracking-[0.05em] border border-ink/30 rounded-full shadow-[0_6px_20px_rgba(14,14,14,0.10)]">
          BELI SEKARANG — Rp {selected.price}
        </a>
      </div>
        ...
        {modal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/48 backdrop-blur-sm"
            ...
            <motion.div
              ...
              className="bg-white w-full max-w-md rounded-3xl p-8 md:p-10 border border-border shadow-2xl overflow-y-auto max-h-[88vh] space-y-6"
            >
              ...
              <button onClick={() => setModal(false)}
                className="w-full bg-ink text-cream py-4 rounded-full font-semibold text-[14px] hover:bg-ink/85 transition-colors cursor-pointer">
                Mengerti
              </button>
            </motion.div>
          </div>
